...
ルートで `pnpm run deploy` を叩くだけ。turbo がフロントのビルドを済ませてから `wrangler deploy` を走らせる。順番が大事で、Workerが `../web/dist` をそのまま配信するので、フロントが古いと古い画面が公開される。

公開先は `https://accounting-app-server.koki-tominaga.workers.dev`。画面とAPIが同じオリジンになる。

デプロイ前に `pnpm check-types` と `pnpm lint` を通しておく。`wrangler deploy` は型エラーを見ないので、壊れたまま上がる。

本番の設定値は2つの場所に分かれている。

| 種類 | 場所 | 備考 |
| --- | --- | --- |
| `WEB_ORIGIN` / `BETTER_AUTH_URL` / `EMAIL_FROM_ADDRESS` | `wrangler.jsonc` の `vars` | 平文でコミット済み |
| `BETTER_AUTH_SECRET` | Cloudflareのシークレット | 設定済み。変えるときは `wrangler secret put` |

データベースの変更は自動で適用されない。スキーマを触ったリリースでは、デプロイと別にマイグレーションを手で当てる。

```sh
pnpm dlx wrangler d1 execute accounting-app-d1 --remote --file=apps/server/drizzle/0004_calm_jack_murdock.sql
```

ファイル名を差し替えて、未適用のものを番号順に実行する。適用済みかどうかを記録する仕組みはないので、どこまで当てたかは自分で覚えておく必要がある。

デプロイ先のアカウントは `wrangler.jsonc` の `account_id` で固定してある。このPCは複数のCloudflareアカウントを使い分けており、`node_modules/.cache/wrangler/` に残った別アカウントのキャッシュをwranglerが優先してしまい、認証エラー(code 10000)でデプロイが通らなくなったことがある。`account_id` はその再発防止なので消さないこと。
...
- **README.md が古い**。「認証・DBは未実装」と書いてあるが、どちらも実装済み。新しい人が最初に読むファイルなので、早めに直したほうがよい
- **CLAUDE.md に2箇所されあり**。ルーターを `BrowserRouter` と書いているが実際は `HashRouter`。また参照先の `note/要件.md` は存在しない
- **使っていない依存**。`react-router` v8 が入っているが、コードは全部 `react-router-dom` からimportしている
- **検証用の残骸**。`apps/server/.wrangler/test-state` は動作確認のために作った隔離データ。git管理外で、消してよい
...
最後に、本番のD1とR2には実データが入っている。`--remote` 付きのコマンドは実行前に内容を確かめること。動作確認はローカルD1で十分にできる。
